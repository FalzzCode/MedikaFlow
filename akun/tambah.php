...
if (is_post()) {
    ...
    if (empty($errors)) {
        try {
            db_execute('INSERT INTO users (nama_lengkap, username, email, password_hash, role, doctor_id, status) VALUES (?, ?, ?, ?, ?, ?, ?)', array(
                $data['nama_lengkap'], $data['username'], $data['email'], password_hash($password, PASSWORD_DEFAULT),
